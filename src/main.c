...
int main(void) {
    ...
    Sound keySound = LoadSound("resources/tab.wav"); // Звук клавиатуры
    Sound doorSound = LoadSound("resources/door.wav"); // Звук двери
    
        const char* sentences[] = {
        "You have $1,000,000 in debt.",
        "You were placed in a gray room alone.",
        "You notice that there are slot machines in this room.",
        "You only have $1,000.",
        "Play to get out.",
    ...
    Image playerup = LoadImage("resources/playerup.png");
    Image playerdown = LoadImage("resources/playerdown.png");
    Image playerright = LoadImage("resources/playerright.png");
    Image playerleft = LoadImage("resources/playerleft.png");
    Texture2D playerTexture = LoadTextureFromImage(playerdown);
    Texture2D slots = LoadTexture("resources/slots.png");
    Texture2D raketa = LoadTexture("resources/rocket.png");
    Texture2D hippodrome = LoadTexture("resources/hippodrome.png");
    Texture2D miner = LoadTexture("resources/miner.png");

    // Player setup
    float speed = 2.75f;
    Vector2 playerPos = {500, 225};
    Vector2 playerLookVect = { 0, 0 };
    Rectangle playerRec = getRect((Vector2) {playerPos.x+10, playerPos.y+10}, playerTexture.width*4.3, playerTexture.height*4.3);  
   ...
   Rectangle topSquare = { 190, 50, miner.height, miner.height };          // Star Game (leftmost)
Rectangle leftSquare = { 1050, 230, raketa.width*1.1, raketa.height*1.75 };     // Rocket Game (center-left)
Rectangle rightSquare = { 700 , 30, 100, 100 };    // Hippodrome Game (center-right)
Rectangle bottomSquare = { 35, 220, slots.width*1.45, slots.height*1.45};  // Slots Game (rightmost)
Rectangle topBorder = { 0, 0, screenWidth, 50};          // Top edge
Rectangle bottomBorder = { 0, screenHeight - 45, screenWidth, 50 }; // Bottom edge
Rectangle leftBorder = { 0, 0, 55, screenHeight };        // Left edge
Rectangle rightBorder = { screenWidth - 65, 0, 90, screenHeight }; // Right edge
Rectangle squares[] = { 
...
Sound horsesRun = LoadSound("resources/horsesrun.mp3");
    ...
    while (!WindowShouldClose()) {
        ...
        if (fadeIn) {
            ...
            if (typingTimer >= typingSpeed) {
                typingTimer = 0.0f;
                currentChar++;
                if (currentChar > strlen(sentences[currentSentence])) {
                    currentChar = strlen(sentences[currentSentence]);
                    fadeTimer = 0.0f;
                    fadeIn = false;
                    waiting = true;
                    StopSound(keySound);
                } else {
                    if (!IsSoundPlaying(keySound)) {
                        PlaySound(keySound);
                    }
                }
            }
            ...
            alpha -= GetFrameTime() / fadeOutTime;
            if (alpha <= 0.0f) {
                alpha = 0.0f;
                fadeOut = false;
        ...
        if (doorPlayed && !IsSoundPlaying(doorSound)) {
            break;
        }

        char displayText[256] = { 0 };
        strncpy(displayText, sentences[currentSentence], currentChar);
        displayText[currentChar] = '\0';

        Color textColor = Fade(WHITE, alpha);
        if (currentSentence == 5) {
            textColor = Fade(RED, alpha);
        }

        DrawText(displayText,
            screenWidth / 2 - MeasureText(sentences[currentSentence], 40) / 2,
            screenHeight / 2 - 20, 40, textColor);
    ...
    if (IsKeyDown(KEY_RIGHT)) {
        ...
        playerPos.x += speed;
        playerRec.x += speed;
        UpdateTexture(playerTexture, ((unsigned char*)playerright.data));
    }
    if (IsKeyDown(KEY_LEFT)) {
        playerLookVect.x = -1;
        playerPos.x -= speed;
        playerRec.x -= speed;
        UpdateTexture(playerTexture, ((unsigned char*)playerleft.data));
    }
    if (IsKeyDown(KEY_UP)) {
        playerLookVect.y = 1;
        playerPos.y -= speed;
        playerRec.y -= speed;
        UpdateTexture(playerTexture, ((unsigned char*)playerup.data));
    ...
    if (IsKeyDown(KEY_DOWN)) {
        ...
        playerPos.y += speed;
        playerRec.y += speed;
        UpdateTexture(playerTexture, ((unsigned char*)playerdown.data));
    }
    ...
    bool isNearLeft = CheckCollisionCircleRec(playerPos, 100, leftSquare);
    ...
    BeginDrawing();
        ...
        DrawTextureEx(slots, (Vector2){45, 200}, 0, 2.95, WHITE);
        DrawTextureEx(miner, (Vector2){175, 30}, 0, 2.75, WHITE);
        DrawTextureEx(raketa, (Vector2){1025, 230}, 0, 2.75, WHITE);
        DrawTextureEx(hippodrome, (Vector2){700, 10}, 0, 2.75, WHITE);
        DrawTextureEx(playerTexture, playerPos, 0, 6, RAYWHITE);
        //DrawRectangleLinesEx(playerRec, 2, RED);
        
        if (isNearTop || isNearLeft || isNearRight || isNearBottom) {
            DrawText("Press E to play", screenWidth / 2 - MeasureText("Press E to play", 26) / 2, 
                    screenHeight / 2 - 20, 26, RED);
        }
    EndDrawing();
    break;
}
    ...
    BeginDrawing();
        ClearBackground(BLACK);
        const char* winText = "YOU ESCAPED THE CASINO!";
        const char* subText = "Press E to return to the main menu";
        
        // Draw centered text
        DrawText(winText, 
            screenWidth/2 - MeasureText(winText, 60)/2, 
            screenHeight/2 - 40, 60, GOLDEN_COLOR);
        DrawText(subText,
            screenWidth/2 - MeasureText(subText, 30)/2,
    ...
    if (IsKeyPressed(KEY_E)) {
        currentGame = MAIN_MENU;
        globalBalance = 1000; // Reset balance if desired
    }
    break;
}
...
if (CheckCollisionPointRec(GetMousePosition(), backButton2) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
    ...
    UnloadTexture(raketa);
    UnloadTexture(slots);
    UnloadTexture(hippodrome);
    UnloadTexture(miner);
